...
$e_page = (int)($_GET['e_page'] ?? 1);
if ($e_page < 1) $e_page = 1;
$e_limit = 10; // Number of items per page
...
if ($e_search !== '') {
    ...
    $count_stmt->execute();
    $total_ebooks = $count_stmt->get_result()->fetch_assoc()['c'];
    $count_stmt->close();
} else {
    $total_ebooks = $db->query("SELECT COUNT(*) c FROM ebooks")->fetch_assoc()['c'];
}
...
<!-- Wide E-Books Repository Section placed cleanly at the bottom -->
<div class="card" style="margin-top: 25px;">
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; margin-bottom:15px;">
        <h3 style="margin:0;"><i class="fa-solid fa-book-atlas"></i> E-Book Catalogue Directory (<?= $total_ebooks ?> Volumes)</h3>
        
        <!-- Search form -->
        <form method="get" style="margin:0; display:flex; gap:8px; width:100%; max-width:400px;">
            <input type="hidden" name="action" value="admin">
            <input type="hidden" name="tab" value="ebooks">
            ...
            <?php if ($e_search !== ''): ?>
                <a href="?action=admin&tab=ebooks" class="btn" style="padding:8px 15px; margin:0; background:var(--accent-orange); display:inline-flex; align-items:center; justify-content:center;" title="Clear Search"><i class="fa-solid fa-arrow-rotate-left"></i></a>
            <?php endif; ?>
        </form>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display:flex; gap:5px;">
                                    <a class="btn" style="background:var(--navy-light); padding:6px 12px; font-size:12px; display:inline-flex; align-items:center; gap:4px;" onclick="openPdfModal(<?= $r['id'] ?>, null, '<?= e(addslashes($r['title'])) ?>', true)"><i class="fa-solid fa-eye"></i> View</a> 
    ...
    <?php if ($total_pages > 1): ?>
        <div class="pagination-container" style="display:flex; justify-content:space-between; align-items:center; margin-top:20px; flex-wrap:wrap; gap:15px; border-top:1px solid var(--border-color); padding-top:15px;">
            ...
            <div class="pagination" style="display:flex; align-items:center; gap:6px;">
                <!-- First Page Button -->
                <?php if ($e_page > 1): ?>
                    <a href="?action=admin&tab=ebooks&e_page=1&e_search=<?= urlencode($e_search) ?>" class="btn" style="padding:6px 10px; background:var(--bg-slate); color:var(--text-color); font-size:12px; display:inline-flex; align-items:center;" title="First Page"><i class="fa-solid fa-angles-left"></i></a>
                    <a href="?action=admin&tab=ebooks&e_page=<?= $e_page - 1 ?>&e_search=<?= urlencode($e_search) ?>" class="btn" style="padding:6px 10px; background:var(--bg-slate); color:var(--text-color); font-size:12px; display:inline-flex; align-items:center; gap:4px;" title="Previous Page"><i class="fa-solid fa-angle-left"></i> Prev</a>
                <?php else: ?>
                ...
                ?>
                    ...
                    <?php else: ?>
                        <a href="?action=admin&tab=ebooks&e_page=<?= $i ?>&e_search=<?= urlencode($e_search) ?>" class="btn" style="padding:6px 12px; background:var(--bg-slate); color:var(--text-color); font-size:12px; border-radius:6px;"><?= $i ?></a>
                    <?php endif; ?>
                <?php endfor; ?>

                <!-- Next Page Button -->
                <?php if ($e_page < $total_pages): ?>
                    <a href="?action=admin&tab=ebooks&e_page=<?= $e_page + 1 ?>&e_search=<?= urlencode($e_search) ?>" class="btn" style="padding:6px 10px; background:var(--bg-slate); color:var(--text-color); font-size:12px; display:inline-flex; align-items:center; gap:4px;" title="Next Page">Next <i class="fa-solid fa-angle-right"></i></a>
                    <a href="?action=admin&tab=ebooks&e_page=<?= $total_pages ?>&e_search=<?= urlencode($e_search) ?>" class="btn" style="padding:6px 10px; background:var(--bg-slate); color:var(--text-color); font-size:12px; display:inline-flex; align-items:center;" title="Last Page"><i class="fa-solid fa-angles-right"></i></a>
                <?php else: ?>
