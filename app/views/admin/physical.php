...
<div class="grid">
    <?php if ($editBook): ?>
        <div class="card">
            <h3><i class="fa-solid fa-user-pen"></i> Edit Physical Book Inventory — <?= e($editBook['title']) ?></h3>
            <form method="post" action="?action=update_physical">
                <?= csrf_input() ?>
                <input type="hidden" name="id" value="<?= $editBook['id'] ?>">
                
                <label for="shelf_number"><i class="fa-solid fa-layer-group"></i>Enter Shelf Number</label>
                <input id="shelf_number" type="number" min="1" max="99" maxlength="2" name="shelf_number" value="<?= e($editBook['shelf_number'] ?? '') ?>" placeholder="Input Shelf Number (1 - 99)" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 2); if(parseInt(this.value, 10) > 99) this.value = '99';">
    ...
    <?php else: ?>
        <div class="card">
            ...
            <form method="post" action="?action=add_physical">
                <?= csrf_input() ?>
                <label for="shelf_number"><i class="fa-solid fa-layer-group"></i> Enter Shelf Number</label>
                <input id="shelf_number" type="number" min="1" max="99" maxlength="2" name="shelf_number" placeholder="Input Shelf Number (1 - 99)" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 2); if(parseInt(this.value, 10) > 99) this.value = '99';">
